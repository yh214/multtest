...
2. Department of Statistics, University of California, Berkeley, {\tt gyc@stat.berkeley.edu}
\end{center}
...
% library(tools)
% Rnwfile<- file.path("/home/sandrine/CVS_stuff/madman/Rpacks/multtest/inst/doc","multtest.Rnw")
...
The dataset {\tt golub} contains the gene expression data for the 38 training set tumor mRNA samples and 3,051 genes retained after pre--processing. The dataset includes
...
We may also wish to look at plots of the numerators and denominators of the test statistics (Figure \ref{fig:mtNumDen})
