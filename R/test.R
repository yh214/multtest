...
MTP<-function(X,W=NULL,Y=NULL,Z=NULL,Z.incl=NULL,Z.test=NULL,na.rm=TRUE,test="t.twosamp.unequalvar",robust=FALSE,standardize=TRUE,alternative="two.sided",psi0=0,typeone="fwer",k=0,q=0.1,fdr.method="conservative",alpha=0.05,nulldist="boot",B=1000,method="ss.maxT",get.cr=FALSE,get.cutoff=FALSE,get.adjp=TRUE,keep.nulldist=FALSE,seed=NULL){
	...
	dx<-dim(X)
	if(length(dx)==0)
		stop("dim(X) must have positive length")
	p<-dx[1]
	n<-dx[2]
	#W
	if(!is.null(W))
	...
	if(!is.null(Y)){
		...
		if(nrow(Y)!=n) 
			stop("Outcome Y has length ",nrow(Y),", not equal to n=",n)
	}
	#Z
	if(!is.null(Z)){
		Z<-as.matrix(Z)
		if(nrow(Z)!=n)
			stop("Covariates in Z have length ",nrow(Z),", not equal to n=",n,"\n")
		#Z.incl tells which columns of Z to include in model
 		if(is.null(Z.incl))
			Z.incl==(1:ncol(Z))
		if(length(Z.incl)>ncol(Z))
			stop("Number of columns in Z.incl ",length(Z.incl)," exceeds ncol(Z)=",ncol(Z))
		if(is.logical(Z.incl))
				Z.incl<-(1:ncol(Z))[Z.incl]
		if(is.character(Z.incl) & length(Z.incl)!=sum(Z.incl%in%colnames(Z)))
			stop(paste("Z.incl=",Z.incl," names columns not in Z",sep=""))
		Za<-Z[,Z.incl]
		...
		if(test=="lm.XvsZ"){
			...
			if(is.logical(Z.test))
				Z.test<-(1:ncol(Z))[Z.test]
			if(is.character(Z.test) & !(Z.test%in%colnames(Z)))
				stop(paste("Z.test=",Z.test," names a column not in Z",sep=""))
			if(is.numeric(Z.test) & !(Z.test%in%(1:ncol(Z))))
					stop("Value of Z.test must be >0 and <",ncol(Z))
			if(Z.test%in%Z.incl){
	...
	#test
	TESTS<-c("t.onesamp","t.twosamp.equalvar","t.twosamp.unequalvar","t.pair","f","f.block","lm.XvsZ","lm.YvsXZ","coxph.YvsXZ")
	test<-TESTS[pmatch(test,TESTS)]
	if(is.na(test))
		stop(paste("Invalid test, try one of ",TESTS,sep=""))
	#robust
	...
	if(is.na(alternative))
		stop(paste("Invalid alternative, try one of ",ALTS,sep=""))
	#null values
	if(length(psi0)>1)
		stop(paste("In current implementation, all hypotheses must have the same null value. Number of null values: ",length(psi0),">1",sep=""))
	#Error rate
	...
	reject<-
		if(nalpha)
			array(dim=c(p,nalpha),dimnames=list(rownames(X),paste("alpha=",alpha,sep="")))
		else
			matrix(nr=0,nc=0)
	if(typeone=="gfwer"){
		if(get.cr==TRUE)
	...
	if(is.na(nulldist))
		stop(paste("Invalid nulldist, try one of ",NULLS,sep=""))
	#methods
	...
	if(test=="f" | test=="f.block"){
		...
		if(!is.null(W))
			warning("Weighted f tests not yet implemented, proceding with unweighted version")
	}
	if(nulldist=="perm"){
		...
		stat.closure<-switch(test,
			...
			t.pair={
				uY<-sort(unique(Y))
				if(length(uY)!=2) 
					stop("Must have two class labels for this test")
				if(trunc(n/2)!=n/2)
					stop("Must have an even number of samples for this test")
				X<-X[,Y==uY[2]]-X[,Y==uY[1]]
		...
		if(method=="ss.maxT")
			out<-ss.maxT(nulldistn,obs,alternative,get.cutoff,get.cr,pind,alpha)
		if(method=="ss.minP")
			out<-ss.minP(nulldistn,obs,rawp,alternative,get.cutoff,get.cr,pind,alpha)
		if(method=="sd.maxT")
			out<-sd.maxT(nulldistn,obs,alternative,get.cutoff,get.cr,pind,alpha)
		if(method=="sd.minP")
			out<-sd.minP(nulldistn,obs,rawp,alternative,get.cutoff,get.cr,pind,alpha)
		if(typeone=="fwer" & nalpha){
			for(a in 1:nalpha)
		...
		out<-new("MTP",statistic=(obs[3,]*obs[1,]/obs[2,]),estimate=(if(ftest) vector("numeric",0) else obs[3,]*obs[1,]),sampsize=n,rawp=rawp,adjp=out$adjp,conf.reg=out$cr,cutoff=out$c,reject=reject,nulldist=nulldistn,call=match.call(),seed=as.integer(seed[1]))
	}
	return(out)
}
...
ss.maxT<-function(null,obs,alternative,get.cutoff,get.cr,get.adjp,alpha=0.05){
	...
	if(get.adjp)
		getp<-apply((obs[1,]/obs[2,])<=matrix(mT,nrow=p,ncol=B,byrow=TRUE),1,mean)
...
sd.maxT<-function(null,obs,alternative,get.cutoff,get.cr,get.adjp,alpha=0.05){
	...
	if(get.cutoff | get.cr){
		getc<-array(dim=c(p,nalpha),dimnames=list(dimnames(null)[[1]],paste("alpha=",alpha,sep="")))
		for(a in 1:nalpha){
			getc[ord[p],a]<-quantile(mT,pr=1-alpha[a])
		}
	}
	if(get.adjp)
		getp[ord[p]]<-mean((obs[1,]/obs[2,])[ord[p]]<=mT)
	for(j in (p-1):1){
		mT<-pmax(mT,null[ord[j],])
		if(get.adjp)
			getp[ord[j]]<-mean((obs[1,ord[j]]/obs[2,ord[j]])<=mT)
		if(get.cutoff | get.cr){
			for(a in 1:nalpha){
				getc[ord[j],a]<-quantile(mT,pr=(1-alpha[a]))
			}					
		}
	}
	...
	for(j in 2:p){
		if(get.adjp)
			getp[ord[j]]<-max(getp[ord[j]],getp[ord[j-1]])
		if(get.cutoff | get.cr){
			for(a in 1:nalpha){
				if(c.ind[a]){
...
fwer2gfwer<-function(adjp,k=0){
        ...
        m<-length(adjp)
	if(any(k>=m))
		stop(paste("number of rejections k=",k," must be less than number of hypotheses=",m,sep=""))
	newp<-NULL
	for(j in k)
		newp<-rbind(newp,c(rep(0,j),adjp[ord[1:(m-j)]]))
  	rownames(newp)<-k
  	colnames(newp)<-ord
...
fwer2fdr<-function(adjp,method="both",alpha=0.05){
	get.cons<-function(adjp,alpha,ord,M,nalpha){
		...
		for(i in 1:nalpha)
			rejections[,i]<-(fwer2tppfp(adjp,a[i])<=a[i])
	...
	get.restr<-function(adjp,alpha,ord,M,nalpha){
		...
		for(m in 1:M){
			...
			if(sum(f<=u)){
				ind<-min(which(f<=u))
				newp[ord[m]]<-if(f[ind]>=l[ind]) ginv(f[ind])
					else ginv(l[ind])
			}
			else
				newp[ord[m]]<-1
